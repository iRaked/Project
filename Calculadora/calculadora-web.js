/**
 * Clase Calculadora adaptada para el navegador
 * Versión web de la calculadora JavaScript
 */
class Calculadora {
    /**
    ...
    formatearNumero(numero) {
        if (numero === Infinity || numero === -Infinity) {
            return numero > 0 ? '∞' : '-∞';
        }
        ...
        if (Math.abs(numero) >= 1e15 || (Math.abs(numero) < 1e-10 && numero !== 0)) {
            return numero.toExponential(6);
        }

        // Redondear a 10 decimales para evitar errores de precisión
        const redondeado = Math.round(numero * 1e10) / 1e10;
        ...
        // Mostrar hasta 10 decimales, eliminando ceros al final
        return redondeado.toString();
    }
}
